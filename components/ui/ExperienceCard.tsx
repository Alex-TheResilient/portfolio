...
export function ExperienceCard({ experience }: ExperienceCardProps) {
  return (
    <div>
      {/* Vertical Line */}
      <div className="ml-2 text-sm border-l border-zinc-300 dark:border-zinc-700">
        <div className="relative ml-2 py-2">
          {/* Vertical Line */}
          <div className="absolute left-3 top-6 bottom-2 w-px bg-zinc-300 dark:bg-zinc-700" />
          {/* Title with Icon */}
          ...
          <div className="ml-8 space-y-3 font-mono text-sm">
            ...
            {/* Description */}
            <ul className="space-y-4 list-disc list-outside pl-4 marker:text-zinc-500">
              {experience.description.map((desc, index) => (
                <li
                  key={index}
                  className="pl-2 text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed"
                >
                  {desc}
                </li>
              ))}
